...
const userArray = ["gustavotremont", "Sol-Zeta", "jjcasco99", "City93", "TaniuOp", "alvaroariasrivero", "Rurucode"];

newUserArray = userArray.map(element => {
    fetch(`https://api.github.com/users/${element}`).then(result => result.json()).then(({name, html_url}) => {
        console.log(name);
        console.log(html_url);
    })
})

Promise.all([...newUserArray]).then(value => console.info("Todas las promesas terminadas", value))